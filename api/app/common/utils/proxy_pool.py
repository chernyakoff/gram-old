...
class ProxyPool:
    ...
    async def acquire_for_account_loading(
        ...
        """
        Получить свободный прокси для нового аккаунта.
        Используем locked_until и lock_session для безопасного распределения между воркерами.
        """
        now = datetime.utcnow()
            ...
            for proxy in available:
                ...
                try:
                    ...
                    if await proxy_model.check():
                        # Очистка lock после успешной проверки
