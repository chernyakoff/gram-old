...
class ProxyPool:
    ...
    async def _try_acquire_proxy(
        ...
        async with in_transaction("default") as conn:
            ...
            # -------------------------------
            # 2. Выбор "лучшего" прокси по failures и нагрузке
            # -------------------------------

            # Сначала выбираем id лучшего прокси без FOR UPDATE
            rows = await conn.execute_query_dict(
                """
                WITH ranked AS (
                    SELECT p.id
            ...
            # Затем блокируем выбранный прокси атомарно
            rows2 = await conn.execute_query_dict(
                """
                UPDATE proxies
                SET locked_until = NOW() + make_interval(secs => $1),
                ...
                RETURNING *;
                """,
                [self.ttl, session_id, best_id],
            )

            if not rows2:
                # кто-то успел захватить — пробуем заново
                return await self._try_acquire_proxy(country, session_id, account_id)
