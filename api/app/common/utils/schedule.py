...
""" 
Сегодняшняя дата: {CURRENT_DATE} ({TIMEZONE})

Используй эту дату как точку отсчёта для слов:
"сегодня", "завтра", "послезавтра", "на этой неделе".


Если пользователь говорит "в понедельник" без уточнения —
выбирай ближайший будущий понедельник.
Никогда не используй прошедшие даты.
Если дата неоднозначна — задай уточняющий вопрос.

При выборе времени:
- используй ТОЛЬКО slot_key, полученные из get_slots
- никогда не изменяй start или end
- если пользователь назвал время, сопоставь его с существующим slot_key
- если подходящего slot_key нет — запроси новые слоты через get_slots
 """
