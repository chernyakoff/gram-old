...
from .enums import AccountAction, DialogStatus, MailingStatus, RecipientStatus, Role
...
class Account(Model, TimestampMixin):
    ...
    recipient_links: fields.ReverseRelation["RecipientAccountLink"]
...
class Recipient(Model):
    ...
    account_links: fields.ReverseRelation["RecipientAccountLink"]
...
class RecipientAccountLink(Model):
    ...
    tg_id = fields.BigIntField(null=False)
    access_hash = fields.BigIntField()  # уникален для пары (recipient, account)
    ...
    class Meta:
        table = "recipient_account_links"
        unique_together = ("recipient_id", "account_id")


class Dialog(Model):
    id = fields.BigIntField(pk=True)

    recipient_account_link = fields.ForeignKeyField(
        "models.RecipientAccountLink",
...
class Message(Model):
    ...
    sender = fields.CharField(max_length=32)  # "account" | "recipient" | "system"
