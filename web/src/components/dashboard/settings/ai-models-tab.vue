  ...
  <UForm ref="form" :schema="aiModelSchema" :state="state" @submit="onSubmit">
    <div class="space-y-6 p-6">
      <div>
        ...
        <p class="text-sm text-gray-500 dark:text-gray-400 mb-4"> Выберите модель для обработки запросов. Цены указаны в рублях. </p>
      </div>
        ...
        <div class="flex items-start gap-4">
          ...
          <div class="flex-1">
            ...
            <p
              ...
              v-html="selectedModel.description"></p>
            <div class="flex flex-wrap gap-4 text-sm">
              <div class="flex items-center gap-2">
                <UIcon
                  name="i-heroicons-arrow-up-circle"
                  class="h-4 w-4 text-primary-500" />
                <span class="text-gray-700 dark:text-gray-200"> Prompt: <strong>{{ formatPrice(selectedModel.promptPrice) }} ₽</strong> /1K токенов </span>
              </div>
              <div class="flex items-center gap-2">
                <UIcon
                  name="i-heroicons-arrow-down-circle"
                  class="h-4 w-4 text-primary-500" />
                <span class="text-gray-700 dark:text-gray-200"> Completion: <strong>{{ formatPrice(selectedModel.completionPrice) }} ₽</strong> /1K токенов </span>
              </div>
            </div>
          </div>
        </div>
      ...
      <div v-else class="space-y-2">
        <p class="text-sm text-gray-500 dark:text-gray-400 mb-3"> Найдено моделей: {{ filteredModels.length }}
        </p>
...
const title = 'Настройки - AiModels'
useTitle(title)
