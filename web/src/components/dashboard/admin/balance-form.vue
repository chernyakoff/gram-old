<template>
    <UForm ref="form" :schema="balanceSchema" :state="state" @submit="onSubmit">
        ...
        <div v-if="response" class="mt-6">
            <UAlert :color="response.status === 'success' ? 'success' : 'error'" :variant="response.status === 'success' ? 'soft' : 'solid'" :title="response.status === 'success' ? 'Успешно' : 'Ошибка'" :description="response.message" :icon="response.status === 'success'
        ...
        <div v-if="error && !response" class="mt-6">
            <UAlert color="error" variant="solid" title="Ошибка" :description="error" icon="i-heroicons-exclamation-triangle" />
        </div>
    </UForm>
</template>
