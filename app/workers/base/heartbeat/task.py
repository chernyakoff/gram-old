...
async def reserve_daily_limit(account_id: int, reserve: int, conn) -> int:
    ...
    await conn.execute_query(
        ...
        """,
        [account_id, orm.AccountAction.NEW_DIALOG, reserve],
    )
...
async def rollback_reserved_daily_limit(account_id: int, reserved: int, conn) -> None:
    ...
    await conn.execute_query(
        ...
        """,
        [account_id, orm.AccountAction.NEW_DIALOG, -reserved],
    )
...
async def lock_account_and_recipients(
    ...
    await (
        orm.Recipient.filter(id__in=ids, status=orm.RecipientStatus.PENDING)
        .filter(Q(lease_expires_at__lt=now) | Q(lease_expires_at__isnull=True))
        .using_db(conn)
        ...
        async with in_transaction() as conn:
            ...
            if active_mailings:
                ...
                active_mailings = [
                    ...
                    if m.status
                    in (orm.MailingStatus.RUNNING, orm.MailingStatus.DRAFT)
                ]
            ...
            for acc in accounts_queue:
                ...
                if in_send_window and has_active_mailings:
                    for mailing in active_mailings:
                        if mailing.status == orm.MailingStatus.DRAFT:
                            if any(r.mailing_id == mailing.id for r in recipients):
                                await (
                                    orm.Mailing.filter(id=mailing.id)
                                    .using_db(conn)
                                    .update(
                                        status=orm.MailingStatus.RUNNING,
                                        started_at=now,
                                    )
                                )
