...
from utils.notify import BotNotify, notify_complete_dialog, send_text_to_user
from utils.prompt import get_name_addon
from workers.dialog.task.session_timer import SessionTimer
...
async def get_last_active_dialog(username: str, account_id: int) -> orm.Dialog | None:
    dialog = (
        ...
        .exclude(status=orm.DialogStatus.COMPLETE)
        .order_by("-started_at")
...
class DialogManager:
    ...
    async def check_and_process_dialogs(self) -> tuple[int, int]:
        ...
        finished_with_pending_system = await orm.Dialog.filter(
            ...
            messages__sender=orm.MessageSender.SYSTEM,
    ...
    async def _process_system_messages_for_dialog(self, dialog: orm.Dialog) -> bool:
        ...
        system_messages = await orm.Message.filter(
            dialog=dialog,
            ui_only=False,
            sender=orm.MessageSender.SYSTEM,
        ...
        try:
            ...
            last_recipient_message = next(
                (
                    ...
                    if m.sender == orm.MessageSender.RECIPIENT
                ),
                None,
            )
    ...
    async def _generate_and_send_response(
        ...
        unsent_system = [
            ...
            if msg.sender == orm.MessageSender.SYSTEM
        ...
        if dialog.status == orm.DialogStatus.COMPLETE:
            self.logger.info(f"[{recipient.username}] AI завершил диалог (COMPLETE)")
            asyncio.create_task(notify_complete_dialog(dialog, self.account))  # type: ignore
            return

        if dialog.status == orm.DialogStatus.NEGATIVE:
            self.logger.info(
                f"[{recipient.username}] AI установил статус NEGATIVE - диалог завершён"
            )
            return

        if dialog.status == orm.DialogStatus.OPERATOR:
            self.logger.info(
    ...
    async def _update_dialog_status(
        ...
        dialog: orm.Dialog,
        recipient: orm.Recipient,
        new_status: orm.DialogStatus | None,
        messages: list[orm.Message],
    ):
        """
        ...
        if new_status and new_status != dialog.status:
            ...
            if new_status in [
                orm.DialogStatus.COMPLETE,
                orm.DialogStatus.NEGATIVE,
                orm.DialogStatus.OPERATOR,
            ]:
            ...
            recipient_messages = [
                m for m in messages if m.sender == orm.MessageSender.RECIPIENT
            ]
            if (
                len(recipient_messages) >= 2
                and dialog.status == orm.DialogStatus.INIT
            ):
                dialog.status = orm.DialogStatus.ENGAGE
                await dialog.save()
                self.logger.info(
    ...
    async def _find_stuck_dialogs(self, min_age_minutes: int = 10) -> list[orm.Dialog]:
        ...
        active_statuses = [
            orm.DialogStatus.INIT,
            orm.DialogStatus.ENGAGE,
            orm.DialogStatus.OFFER,
            orm.DialogStatus.CLOSING,
        ]
        ...
        for dialog in dialogs:
            ...
            if (
                last_message.sender == orm.MessageSender.RECIPIENT
    ...
    async def _check_morning_reminders(self):
        ...
        for meeting in meetings:
            ...
            if success:
                dialog.status = orm.DialogStatus.CLOSING
                await dialog.save(update_fields=["status"])
