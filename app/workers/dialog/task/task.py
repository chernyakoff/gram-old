    ...
    try:
        ...
        try:
            ...
            if "used under two different IP" in str(e):
                ...
                account.status = orm.AccountStatus.EXITED
        ...
        followups_sent = await manager.check_and_send_follow_ups()
        if followups_sent > 0:
            logger.info(f"Follow-up сообщений отправлено: {followups_sent}")
        ...
        account.status = orm.AccountStatus.FROZEN
        ...
        account.status = orm.AccountStatus.MUTED
        await account.save(
